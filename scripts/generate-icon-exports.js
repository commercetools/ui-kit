...
iconsToProcess.forEach((data) => {
  glob(data.svgPath, async (err, files) => {
    ...
    const isExternalIconsPackage = relativeStyledIconPath.includes(
      'icons/src/create-styled-icon'
    );
    const createStyledIconImportPath = isExternalIconsPackage
      ? '@commercetools-uikit/icons'
      : relativeStyledIconPath;
    ...
    const importStatements = files.reduce(
      (importsString, fileName) => {
        const fileNameWithoutExtension = path.basename(fileName, iconFileExt);
        const componentName = upperFirst(camelCase(fileNameWithoutExtension));
      ...
      isExternalIconsPackage ? '' : `export * from './create-styled-icon';`
    );
