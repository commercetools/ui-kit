...
const loadTemplateCreateStyledIcon = (opts) =>
  fs.readFileSync(
    path.join(
      __dirname,
      `templates/icon.styles.${opts.typescript ? 'tsx' : 'js'}`
    ),
    { encoding: 'utf8' }
  );
...
const styledIconsTemplate = (
  ...
  const templateCreateStyledIcon = loadTemplateCreateStyledIcon(opts);
...
${opts.typescript ? '' : `${reactComponentName}.propTypes = iconPropTypes;`}
