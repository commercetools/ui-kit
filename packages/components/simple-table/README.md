...
| `cellAlignment` | `string`          | -        | `left`                               | The default cell horizontal alignment. Can be one of [`left`, `center`, `right`]. Each column definition may have its own specific alignment, which will override this one.                                                                                                             |
...
| `onSortChange`  | `func`            | -        | -                                    | Function called when a sortable column's header is clicked. Required if you set `isSortable` on at least on column. Should implement the following interface: (columnKey: string, sortDirection: string): void                                                                          |
...
| `align`                | `string`  | -        | -       | Use this to override the table's own `cellAlignment` prop for this specific column. Can be one of [`left`, `center`, `right`].                                                                                                                                                                                                                                                                                                                                        |
...
| `onClick`              | `func`    | -        | -       | Function to make the cells of this column clickable by the user. Keep in mind that if used on a row that implements `onRowClick`, you might want to set the column property `shouldIgnoreRowClick` to ignore it for this column, otherwise, upon click this function will be called first and then `onRowClick` immediately afterwards.                                                                                                                               |
...
| `isTruncated`          | `boolean` | -        | `false` | Set this to `true` to allow text content of this cell to be truncated with an ellipsis, instead of breaking into multiple lines.                                                                                                                                                                                                                                                                                                                                      |
...
| `shouldIgnoreRowClick` | `boolean` | -        | `false` | Set this to `true` to prevent click event propagation for this cell. You might want this if you need the column to have its own `onClick` while the row also has a defined `onRowClick`.                                                                                                                                                                                                                                                                              |
