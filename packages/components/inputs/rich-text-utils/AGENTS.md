...
- **HTML module**: Serializes Slate descendants → HTML and deserializes HTML →
  Slate descendants. Uses `dompurify` for XSS sanitization on deserialization
...
- **RichTextBody**: The toolbar UI component with formatting buttons (bold,
  italic, underline, strikethrough, superscript, subscript, code, headings,
  lists, links). Uses `downshift` for the "more styles" dropdown.
...
- This package has its own SVG icons in `src/icons/svg/` that are code-genned
