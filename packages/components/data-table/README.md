...
```ts
(args: TColumn<RowData>[]) => void
```

### Signature `itemRenderer`

```ts
(item: TRow, column: TColumn<RowData>, isRowCollapsed: boolean) => ReactNode;
```
