import React from 'react';

const Export = props => (
  <svg width={24} height={24} viewBox="0 0 24 24" {...props}>
    <g id="export__Icons" stroke="none" strokeWidth={1} fillRule="evenodd">
      <g id="export__MC-icon-set" transform="translate(-240 -648)" fill="#000">
        <g id="export__Actions" transform="translate(24 648)">
          <g id="export__Export" transform="translate(216)">
            <path
              d="M9.954 14C9.427 14 9 13.543 9 13c0-.542.427-1 .954-1H17V9l4 4-4 4v-3H9.954zM7.414 3H16a1 1 0 011 1v3a1 1 0 01-2 0V5h-5v5H5v9h10a1 1 0 012 0v1a1 1 0 01-1 1H4a1 1 0 01-1-1V7.414a1 1 0 01.293-.707l3.414-3.414A1 1 0 017.414 3zM5 8h3V5h-.172L5 7.828V8z"
              id="export__shape"
            />
          </g>
        </g>
      </g>
    </g>
  </svg>
);

export default Export;
